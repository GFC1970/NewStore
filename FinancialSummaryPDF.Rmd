...
Revenue amounts shown in £ GBP Sterling
...
t <- data_tbl %>% 
  ...
  group_by(yr, mth, product_category) %>% 
  ...
  mutate(yr = paste0("Yr", yr) %>% as_factor()) %>% 
  spread(key = yr, value = revenue) %>% 
  rename(Month = mth, Category = product_category)


t1 <- gt(t, groupname_col = "Category") %>% 
  fmt_number(columns = vars(Yr2015,Yr2016,Yr2017,Yr2018,Yr2019,Yr2020),
               decimals = 0)

t2 <- t1 %>% tab_style(
  style = cell_text(weight = "bold"),
    locations = cells_body(
      columns =vars(Yr2015,Yr2016,Yr2017,Yr2018,Yr2019,Yr2020))
)
 
t3 <- t2 %>% tab_options(table.font.names = "Roboto Condensed",
